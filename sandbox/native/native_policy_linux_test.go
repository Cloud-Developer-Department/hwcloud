...
func TestBwrapArgsNetworkPolicy(t *testing.T) {
	cmd := hwcloud.Command{Program: "echo"}
...
func TestBwrapArgsEtcMounts(t *testing.T) {
	...
	args, ok := sb.bwrapArgs(hwcloud.Command{Program: "echo"})
	if !ok {
		t.Skip("bwrap not installed")
	}
...
func TestBwrapArgsExtraPaths(t *testing.T) {
	sb, err := NewWithPolicy(t.TempDir(), Policy{
		WritablePaths: []string{"/tmp/hwcloud-wtest"},
	...
	foundReadable := false
	for i, a := range args {
		if a == "--bind" && i+2 < len(args) && args[i+1] == "/tmp/hwcloud-wtest" && args[i+2] == "/tmp/hwcloud-wtest" {
			foundWritable = true
		}
	...
	if !foundWritable {
		t.Errorf("expected writable bind for /tmp/hwcloud-wtest, args: %v", args)
	} else {
