...
func sandboxFunctional(t *testing.T) bool {
	...
	result, err := sb.Run(context.Background(), hwcloud.Command{
		Program: "/bin/echo",
		Args:    []string{"probe"},
	})
	if err != nil {
...
func TestSandboxBlocksExternalAccess(t *testing.T) {
	...
	result, _ := sb.Run(context.Background(), hwcloud.Command{
		Program: "/bin/bash",
...
func TestSandboxStreaming(t *testing.T) {
	...
	ch := sb.RunStream(context.Background(), &hwcloud.Command{
		Program: "/bin/bash",
