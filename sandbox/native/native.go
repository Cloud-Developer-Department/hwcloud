...
// CWD implements hwcloud.Sandbox. Returns the path as seen from
...
func (s *Sandbox) Run(ctx context.Context, cmd hwcloud.Command) (hwcloud.Result, error) {
	if !s.policy.Enabled {
...
func (s *Sandbox) RunStream(ctx context.Context, cmd *hwcloud.Command) <-chan hwcloud.ToolStreamChunk {
	if !s.policy.Enabled {
...
func readLines(r io.Reader, ch chan<- hwcloud.ToolStreamChunk, done chan<- struct{}) {
	defer func() { done <- struct{}{} }()
	sc := bufio.NewScanner(r)
	sc.Buffer(make([]byte, 4096), 1024*1024)
	for sc.Scan() {
		ch <- hwcloud.ToolStreamChunk{Content: sc.Text() + "\n"}
	}
}
...
func (co *cmdOutput) configure(c *exec.Cmd, cmd hwcloud.Command) {
	if cmd.StdoutW != nil {
...
// result builds an hwcloud.Result for a completed process.
// Caller must call readFiles first.
func (co *cmdOutput) result(pid int) hwcloud.Result {
...
// partialResult builds an hwcloud.Result for a still-running process
// (ctx expired before the process exited). ExitCode is set to -1 and no
// exit code file is written — the process is still alive.
func (co *cmdOutput) partialResult(pid int) hwcloud.Result {
	r := co.result(pid)
	r.ExitCode = -1
	return r
}
...
func writeExitCode(err error, cmd hwcloud.Command) {
...
func (s *Sandbox) unconfinedRun(ctx context.Context, cmd hwcloud.Command) (hwcloud.Result, error) {
	...
	if err := c.Start(); err != nil {
		return hwcloud.Result{}, fmt.Errorf("native sandbox (unconfined): %w", err)
	}
...
func (s *Sandbox) unconfinedRunStream(ctx context.Context, cmd *hwcloud.Command) <-chan hwcloud.ToolStreamChunk {
