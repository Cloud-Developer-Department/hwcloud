...
// stubModel is a minimal hwcloud.Model with a configurable ContextWindow
// and a recorded modelID, used to verify which model a team run uses.
type stubModel struct {
	name    string
	cw      int
	called  int
	lastReq hwcloud.ChatCompletionRequest
}

func (m *stubModel) ChatCompletion(ctx context.Context, req hwcloud.ChatCompletionRequest) (*hwcloud.ChatCompletionResponse, error) {
	m.called++
	m.lastReq = req
	return &hwcloud.ChatCompletionResponse{
		Choices: []hwcloud.Choice{{
			Message:      hwcloud.Message{Role: hwcloud.RoleAssistant, Content: "ok"},
			FinishReason: "stop",
		}},
	}, nil
}

func (m *stubModel) ChatCompletionStream(ctx context.Context, req hwcloud.ChatCompletionRequest) (hwcloud.StreamReader, error) {
	m.called++
	m.lastReq = req
	return nil, nil
}
