// Package acp provides hwcloud Agent ↔ ACP protocol integration.
//
// AgentServer wraps an [hwcloud.Agent] as an [openacp.AgentHandler],
...
import (
	...
	"github.com/Cloud-Developer-Department/hwcloud/mcp"
...
type AgentServer struct {
	...
	ToolFactory func(cwd string) []hwcloud.Tool
...
func (ss *agentSession) setSubAgentTools(tools []hwcloud.Tool) {
	ss.modeMu.Lock()
	defer ss.modeMu.Unlock()
...
func NewAgentServer(cfg *agent.Agent, deps kernel.Deps, store session.Store, models map[string]hwcloud.Model) *AgentServer {
	s := &AgentServer{
		Cfg:          cfg,
		Deps:         deps,
...
func (s *AgentServer) switchSessionModel(ss *agentSession, m hwcloud.Model) {
...
// contentBlocksToMessage converts ACP ContentBlocks to an hwcloud.Message.
// Text blocks become message content; images and resources become ContentParts
// so the model backend can handle them natively.
func (s *AgentServer) contentBlocksToMessage(blocks []openacp.ContentBlock) (hwcloud.Message, error) {
	var textParts []string
	var contentParts []hwcloud.ContentPart
	hasText := false
	...
	for _, b := range blocks {
		...
		case "image":
			...
			if b.Data != "" && b.MimeType != "" {
				contentParts = append(contentParts, hwcloud.ContentPart{
					Type: "image_url",
					ImageURL: &hwcloud.ImageURL{
...
func toolNames(tools []hwcloud.Tool) []string {
...
func (s *AgentServer) executionTools(sid openacp.SessionId, ss *agentSession) []hwcloud.Tool {
	var add []hwcloud.Tool
...
func (a *acpApprover) Ask(ctx context.Context, call hwcloud.ToolCall, def hwcloud.FunctionDefinition, session hwcloud.Session) (governance.Decision, error) {
	if a.client == nil {
...
func (s *AgentServer) usageCost(ss *agentSession, usage hwcloud.Usage) *openacp.Cost {
	s.modelsMu.Lock()
	key := s.defaultModelID
	if val, ok := ss.ConfigString("model"); ok {
		key = val
...
func (p planModeStub) Definition() hwcloud.FunctionDefinition {
	return p.def
}

func (p planModeStub) Execute(_ context.Context, _ json.RawMessage) *hwcloud.ToolResult {
