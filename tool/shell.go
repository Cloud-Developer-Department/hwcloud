// Package tool provides built-in Tool implementations for hwcloud.
package tool
...
// Shell lets an agent execute shell commands inside an [hwcloud.Sandbox].
...
//
// Implements both [hwcloud.Tool] and [hwcloud.StreamExecutor].
type Shell struct {
	sandbox hwcloud.Sandbox
}

func NewShell(sandbox hwcloud.Sandbox) *Shell {
...
func (t *Shell) Definition() hwcloud.FunctionDefinition {
	...
	return hwcloud.FunctionDefinition{
		Name:        "shell",
		Description: desc,
		Parameters:  hwcloud.SchemaOf[ShellParams](),
	}
}

func (t *Shell) Execute(ctx context.Context, args json.RawMessage) *hwcloud.ToolResult {
	params, err := hwcloud.ParseArgs[ShellParams](args)
	if err != nil {
		return hwcloud.ErrorResult(fmt.Errorf("shell: %w", err), false, "")
	}
	if t.sandbox == nil {
		return hwcloud.ErrorResult(fmt.Errorf("shell: no sandbox configured"), false, "")
	}
	...
	if pm != nil {
		...
		if errors.Is(runErr, hwcloud.ErrProcessRunning) {
			// Process still running — rename dir to proc-{PID} and return snapshot.
			proc.SetPID(result.PID)
			return &hwcloud.ToolResult{Content: formatProcessRunning(proc)}
		}
		// Process finished — clean up and return result.
		proc.Close()
		pm.Remove(proc.ID)
		if runErr != nil {
			return hwcloud.ErrorResult(runErr, false, "")
		}
		return &hwcloud.ToolResult{Content: formatShellResult(result)}
	}

	// No ProcessManager — use sandbox directly (preserves backward compat).
	result, err := t.sandbox.Run(shellCtx, cmd)
	if errors.Is(err, hwcloud.ErrProcessRunning) {
		return &hwcloud.ToolResult{Content: formatProcessRunningNoFiles(result)}
	}
	if err != nil {
...
func (t *Shell) ExecuteStream(ctx context.Context, args json.RawMessage) <-chan hwcloud.ToolStreamChunk {
	params, err := hwcloud.ParseArgs[ShellParams](args)
	if err != nil {
		ch := make(chan hwcloud.ToolStreamChunk, 1)
		ch <- hwcloud.ToolStreamChunk{Error: fmt.Errorf("shell: %w", err)}
		close(ch)
		return ch
	}
	if t.sandbox == nil {
		ch := make(chan hwcloud.ToolStreamChunk, 1)
		ch <- hwcloud.ToolStreamChunk{Error: fmt.Errorf("shell: no sandbox configured")}
		close(ch)
		return ch
	}
	...
	type streamRunner interface {
		RunStream(ctx context.Context, cmd *hwcloud.Command) <-chan hwcloud.ToolStreamChunk
	}
	...
	if !ok {
		...
		go func() {
			...
			if output.Error != nil {
				ch <- hwcloud.ToolStreamChunk{Error: output.AsError()}
			} else {
				ch <- hwcloud.ToolStreamChunk{Content: output.Content}
			}
		}()
		return ch
	}
	...
	wrapped := make(chan hwcloud.ToolStreamChunk, 16)
	go func() {
		defer cancel()
		defer close(wrapped)
		for {
			...
			case chunk, ok := <-src:
				...
				case <-streamCtx.Done():
					if proc != nil && cmd.PID > 0 {
						proc.SetPID(cmd.PID)
						select {
						case wrapped <- hwcloud.ToolStreamChunk{Content: formatProcessRunning(proc)}:
						default:
						}
					}
					go func() {
...
func formatShellResult(result hwcloud.Result) string {
...
func formatProcessRunningNoFiles(result hwcloud.Result) string {
	var b strings.Builder
