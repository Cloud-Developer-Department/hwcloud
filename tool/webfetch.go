...
func (t *WebFetch) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{
		...
		Parameters: hwcloud.SchemaOf[WebfetchParams](),
	}
}

func (t *WebFetch) Execute(ctx context.Context, args json.RawMessage) *hwcloud.ToolResult {
	params, err := hwcloud.ParseArgs[WebfetchParams](args)
	if err != nil {
		return hwcloud.ErrorResult(fmt.Errorf("%s: %w", webFetchName, err), false, "")
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return hwcloud.ErrorResult(fmt.Errorf("%s: HTTP %d for %s: %s", webFetchName, resp.StatusCode, utils.ScrubURL(params.URL), snippet), false, "")
	}
