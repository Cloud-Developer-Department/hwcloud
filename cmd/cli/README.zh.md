...
[hwcloud](https://github.com/Cloud-Developer-Department/hwcloud) 的命令行入口。通过配置驱动模型和 WASM 插件，启动一个 LLM Agent 服务器（REST 或 ACP）。
...
所有配置存放在 `~/.hwcloud/settings.json` 中。可通过 `HWCLOUD_CLI_CONFIG` 环境变量覆盖文件路径。
...
| `plugins` | 否 | 插件目录或 `.wasm` 文件列表（默认 `~/.hwcloud/plugins`） |
| `env` | 否 | 进程静态环境变量 |
