...
The command-line entry point for [hwcloud](https://github.com/Cloud-Developer-Department/hwcloud). Start an LLM agent server (REST or ACP) with config-driven models and WASM-powered plugins.
...
mkdir -p ~/.hwcloud/plugins
```

Create `~/.hwcloud/settings.json`:
...
All configuration lives in `~/.hwcloud/settings.json`. The path can be overridden via `HWCLOUD_CLI_CONFIG` env var.
...
| `plugins` | No | Plugin directories or `.wasm` files (default: `~/.hwcloud/plugins`) |
...
Plugins are built in Rust with the `hwcloud-pdk` crate:

```bash
# Build the SDK
cd cmd/cli
cargo build --release --target wasm32-unknown-unknown -p hwcloud-pdk
...
```rust
#![no_std]
extern crate hwcloud_pdk as sdk;
use sdk::prelude::*;
...
pub extern "C" fn init(p: u32, l: u32) -> u64 {
    ...
    let ak = host::keyring_get("hwcloud", "my_provider_api_key").unwrap_or("");
