...
func streamReply(reply channel.ReplyFunc, stream <-chan hwcloud.StreamEvent, sessionID string, updater runCardUpdater, sizer CardSizer) {
	type tpend struct {
		name string
	}
...
loop:
	for {
		...
		case evt, ok := <-stream:
			...
			switch evt.Type {
			case hwcloud.StreamToolResult, hwcloud.StreamTextDelta,
				hwcloud.StreamRetrying, hwcloud.StreamDone,
				hwcloud.StreamError, hwcloud.StreamAborted:
				clearApproval()
			}
			switch evt.Type {
