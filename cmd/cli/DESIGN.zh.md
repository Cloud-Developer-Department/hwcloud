...
hwcloud 是 hwcloud 的命令行入口。不是简单的启动脚本，而是一个**可扩展的 Agent 运行时平台**：

1. 配置来自 `~/.hwcloud/settings.json`
...
| `plugins` | 否 | 插件目录或文件列表；默认 `~/.hwcloud/plugins` |
...
编译：`cargo build --release --target wasm32-unknown-unknown -p hwcloud-pdk && rustc ... --extern hwcloud_pdk`
...
cmd/cli/
  ...
  config/config.go             settings 加载器、默认值、HWCLOUD_CLI_CONFIG 环境变量
