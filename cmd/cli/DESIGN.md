...
hwcloud is the CLI entry point for hwcloud. It is not a simple launch script — it is an **extensible agent runtime platform**:

1. Configuration from `~/.hwcloud/settings.json`
...
| `plugins` | No | List of dirs/files; default `~/.hwcloud/plugins` |
...
Plugins are built with the `hwcloud-pdk` crate. The SDK provides:
...
pub extern "C" fn init(p: u32, l: u32) -> u64 {
    ...
    let ak = host::keyring_get("hwcloud", "my_key").unwrap_or("");
    // ... build merged JSON ...
    sdk_return(merged.as_bytes())
}
```

Build: `cargo build --release --target wasm32-unknown-unknown -p hwcloud-pdk && rustc ... --extern hwcloud_pdk`
...
cmd/cli/
  ...
  config/config.go             Settings loader, defaults, HWCLOUD_CLI_CONFIG env var
  ...
  plugin/pdk/rust/                    Rust SDK crate (hwcloud-pdk)
  examples/plugin/
