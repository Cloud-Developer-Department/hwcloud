...
// ~/.<version.Name> (e.g. ~/.hwcloud), set at build time via ldflags;
// HWCLOUD_CLI_CONFIG overrides the settings.json location entirely.
func Path() string {
	...
	if p := os.Getenv("HWCLOUD_CLI_CONFIG"); p != "" {
		info, err := os.Stat(p)
		if info != nil && info.IsDir() {
			fmt.Fprintf(os.Stderr, "HWCLOUD_CLI_CONFIG is a dir(%v), use: %s\n", p, defaultPath)
		}
		if err != nil {
			fmt.Fprintf(os.Stderr, "HWCLOUD_CLI_CONFIG err: %v\n", err)
...
// settings.json (HWCLOUD_CLI_CONFIG or the default). Every persistent
// path (profile, plugins, channel state) derives from this single root,
// so pointing HWCLOUD_CLI_CONFIG at a custom settings file relocates
