...
impl Plugin for ExtendedSettings {
    ...
    fn init(settings_json: &str) -> Result<String, String> {
        let ak = host::keyring_get("hwcloud", "my_provider_api_key");
        let bu = host::keyring_get("hwcloud", "my_provider_base_url");
        if ak.is_err() || bu.is_err() {
            return Ok(settings_json.into());
        }
        let ak = ak.unwrap();
        let bu = bu.unwrap();
        let mdls = host::keyring_get("hwcloud", "my_provider_models").ok();
        Ok(inject_settings(settings_json, &ak, &bu, mdls.as_deref()))
    }
}

hwcloud_pdk::export!(ExtendedSettings);
