...
impl Plugin for StatsCmd {
    ...
    fn run_command(_name: &str, input: &CommandInput) -> Result<String, String> {
        ...
        let has_ak = host::keyring_get("hwcloud", "my_provider_api_key").is_ok();
        let has_bu = host::keyring_get("hwcloud", "my_provider_base_url").is_ok();
...
hwcloud_pdk::export!(StatsCmd);

#[no_mangle]
pub extern "C" fn run_stats(ptr: u32, len: u32) -> u64 {
    hwcloud_pdk::dispatch_command::<StatsCmd>(ptr, len, "stats")
}
