...
type SafetyClassifier interface {
	Classify(def hwcloud.FunctionDefinition) SafetyClass
}
...
type HumanApprover interface {
	Ask(ctx context.Context, call hwcloud.ToolCall, def hwcloud.FunctionDefinition, session hwcloud.Session) (Decision, error)
}
...
type Policy interface {
	Evaluate(ctx context.Context, call hwcloud.ToolCall, def hwcloud.FunctionDefinition, session hwcloud.Session) (Decision, error)
}
...
type Engine struct {
	...
	DecObserver hwcloud.DecisionObserver
}
...
func (e *Engine) WithDecisionObserver(obs hwcloud.DecisionObserver) *Engine {
	e.DecObserver = obs
	return e
}
...
// with the same value.
func matchesRule(rule Rule, call hwcloud.ToolCall) bool {
	if rule.ToolPattern == "" {
		return false
	}
	p := rule.ToolPattern
...
func (e *Engine) Evaluate(ctx context.Context, call hwcloud.ToolCall, def hwcloud.FunctionDefinition, session hwcloud.Session) (Decision, error) {
	...
	emit := func(layer, outcome string, detail map[string]any) {
		if e.DecObserver != nil {
			e.DecObserver.ObserveDecision(ctx, hwcloud.DecisionEvent{
	...
	for _, rule := range e.Rules {
		...
		if rule.Action == Ask {
			emit(hwcloud.DecisionPolicyRule, hwcloud.OutcomeAsk, map[string]any{"reason": rule.Reason})
			return e.askHuman(ctx, call, def, session, rule.Reason)
		}
		emit(hwcloud.DecisionPolicyRule, string(rule.Action), map[string]any{"reason": rule.Reason})
	...
	if len(e.Rules) > 0 {
		emit(hwcloud.DecisionPolicyRule, hwcloud.OutcomeSkipped, nil)
	}

	// 2) Safety layer: read-only tools auto-allow.
	if e.Safety != nil {
		if e.Safety.Classify(def) == ReadOnly {
			emit(hwcloud.DecisionPolicySafety, hwcloud.OutcomeAllow, map[string]any{"classifier": "readonly"})
			return Decision{Action: Allow, Reason: "read-only tool"}, nil
		}
		emit(hwcloud.DecisionPolicySafety, hwcloud.OutcomeSkipped, nil)
	}
	...
	if e.Memory != nil {
		if keys := MemoryKeys(call.Function.Name, json.RawMessage(call.Function.Arguments)); len(keys) > 0 {
			...
			if all {
				emit(hwcloud.DecisionPolicyMemory, hwcloud.OutcomeHit, map[string]any{"mode": "multi-key", "keys": len(keys)})
				return Decision{Action: Allow, Reason: "remembered"}, nil
			}
			emit(hwcloud.DecisionPolicyMemory, hwcloud.OutcomeMiss, map[string]any{"mode": "multi-key", "keys": len(keys)})
		} else {
			key := ApprovalKey(call.Function.Name, json.RawMessage(call.Function.Arguments))
			if d, ok := e.Memory.Recall(ctx, session.ID, key); ok {
				if d.Action == Ask {
					emit(hwcloud.DecisionPolicyMemory, hwcloud.OutcomeAsk, map[string]any{"mode": "single"})
					return e.askHuman(ctx, call, def, session, "remembered ask")
				}
				emit(hwcloud.DecisionPolicyMemory, hwcloud.OutcomeHit, map[string]any{"mode": "single", "action": string(d.Action)})
				return d, nil
			}
			emit(hwcloud.DecisionPolicyMemory, hwcloud.OutcomeMiss, map[string]any{"mode": "single"})
		}
	}
...
func (e *Engine) askHuman(ctx context.Context, call hwcloud.ToolCall, def hwcloud.FunctionDefinition, session hwcloud.Session, reason string) (Decision, error) {
	if e.DecObserver != nil {
		ri := hwcloud.RunInfoFromContext(ctx)
		outcome := hwcloud.OutcomeAsk
		if e.Human == nil {
			outcome = hwcloud.OutcomeDeny
		}
		e.DecObserver.ObserveDecision(ctx, hwcloud.DecisionEvent{
			Layer: hwcloud.DecisionPolicyHuman, Outcome: outcome,
...
func (c *ToolClassifier) Classify(def hwcloud.FunctionDefinition) SafetyClass {
