...
Create `~/.hwcloud/settings.json` (the `hwcloud` leaf is `version.Name`, default `hwcloud`; a build injected with `-X ...version.Name=myagent` reads `~/.myagent/` instead):
...
Put `AGENTS.md` and `SOUL.md` in the profile directory (default `~/.hwcloud/profile/`; when `HWCLOUD_CLI_CONFIG` points settings elsewhere, `<config-dir>/profile/` next to it) to customise the agent's behaviour. Project-level `$(pwd)/AGENTS.md` overrides it.

Connect an ACP client (VSCode/Zed plugin).

#### Web Search backend

The `websearch` tool supports two backends, selected by `HWCLOUD_WEB_SEARCH_ENGINE`:
...
```bash
export HWCLOUD_WEB_SEARCH_ENGINE=bocha
...
{
  "log": {
    "file": "/var/log/hwcloud/hwcloud.log",
    "max_size": 10,
    "max_backups": 5,
    "level": "debug"
  }
...
All fields are optional. Defaults: `~/.hwcloud/logs/hwcloud.log`, 10 MB rotation, 5 backups, info level.
...
A plugin declares its type(s) via metadata. A single module can implement multiple types (comma-separated, e.g. `"cli:settings,cli:http"`). We provide a Rust SDK (`plugin/pdk/rust/`, crate `hwcloud-pdk`) that wraps the FFI contract via the `Plugin` trait + `export!` macro — but the ABI is simple enough to implement from any language.
...
cp target/wasm32-unknown-unknown/release/example_agent_tool.wasm ~/.hwcloud/plugins/echo.wasm
```
...
hwcloud_pdk::export!(LoggerPlugin);
```
...
| Package | Purpose |
|---------|---------|
| `hwcloud` | Core types — Agent (pure config), Team, ToolResult, token helpers |
...
| `plugin/pdk/rust/` | Rust SDK crate (`hwcloud-pdk`) for building WASM plugins |
