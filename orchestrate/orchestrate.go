...
type Plan struct {
	...
	model      hwcloud.Model // for step output summarisation
	config     PlanConfig
...
func WithModel(model hwcloud.Model) PlanOption {
...
func (p *Plan) Plan(ctx context.Context, goal string, history []hwcloud.Message) (*PlanDef, error) {
	if p.planner == nil {
...
func (p *Plan) PlanStream(ctx context.Context, goal string, history []hwcloud.Message, onChunk func(string)) (*PlanDef, error) {
	if p.planner == nil {
		return nil, fmt.Errorf("plan: no Planner configured")
	}
	if len(p.agents) == 0 {
	...
	type streamPlanner interface {
		PlanStream(ctx context.Context, goal string, agents []agent.AgentInfo, history []hwcloud.Message, onChunk func(string)) (*PlanDef, error)
	}
...
func (p *Plan) Execute(ctx context.Context, session hwcloud.Session, def *PlanDef) (*PlanResult, error) {
	if p.approver != nil {
...
func (p *Plan) ExecuteStream(ctx context.Context, session hwcloud.Session, def *PlanDef) <-chan PlanEvent {
...
// (for replan).
func (p *Plan) ExecuteWithState(ctx context.Context, session hwcloud.Session, def *PlanDef, state *PlanState) <-chan PlanEvent {
	ch := make(chan PlanEvent, 32)
	go func() {
		defer close(ch)
		...
		resumeCtx := hwcloud.WithRunInfo(ctx, hwcloud.RunInfo{
...
func (p *Plan) Run(ctx context.Context, session hwcloud.Session, goal string, history []hwcloud.Message) (*PlanResult, error) {
...
func (p *Plan) RunStream(ctx context.Context, session hwcloud.Session, goal string, history []hwcloud.Message) <-chan PlanEvent {
	ch := make(chan PlanEvent, 32)
	go func() {
		defer close(ch)
		def, err := p.Plan(ctx, goal, history)
...
func (p *Plan) execute(ctx context.Context, session hwcloud.Session, def *PlanDef, eventCh chan<- PlanEvent) (*PlanResult, error) {
	now := time.Now()
	...
	planRunID := uuid.NewString()
	prev := hwcloud.RunInfoFromContext(ctx)
	ctx = hwcloud.WithRunInfo(ctx, hwcloud.RunInfo{
		RunID: planRunID, TurnID: -1, ParentRunID: prev.RunID,
	})
	state := &PlanState{
		ID:        session.ID + "/plan",
