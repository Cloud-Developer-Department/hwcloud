...
func (o *progressObserver) ObserveStage(ctx context.Context, event hwcloud.StageEvent) {
	if event.Name == hwcloud.StageToolExecute && event.Phase == "enter" {
		name, _ := event.Detail["tool"].(string)
		fmt.Printf("  → running %s...\n", name)
	}
	if event.Name == hwcloud.StageToolExecute && event.Phase == "leave" {
		fmt.Printf("    done (%.1fs)\n", event.Duration.Seconds())
	}
}

var _ hwcloud.RunObserver = (*progressObserver)(nil)
...
Step 6: Do NOT call iac_apply unless the user explicitly asks for it.`),
	...
	result, err := kernel.New(cfg, deps).Run(ctx, sess, hwcloud.UserMessage(goal))
	if err != nil {
