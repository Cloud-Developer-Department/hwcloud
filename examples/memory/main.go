...
import (
	...
	memorysqlite "github.com/Cloud-Developer-Department/hwcloud/provider/memory/sqlite"
	sessionsqlite "github.com/Cloud-Developer-Department/hwcloud/session/sqlite"
	summarizer "github.com/Cloud-Developer-Department/hwcloud/summarizer"
	opentool "github.com/Cloud-Developer-Department/hwcloud/tool"
)

type compactionObserver struct{}

func (o *compactionObserver) ObserveStage(ctx context.Context, event hwcloud.StageEvent) {
	if event.Name != hwcloud.StageMemoryFetch || event.Phase != "leave" {
		return
	}
	d := event.Detail
	if d == nil {
...
var _ hwcloud.RunObserver = (*compactionObserver)(nil)
...
You have access to a workspace where you can create and modify files.`),
	...
	rt.Run(ctx, session, hwcloud.UserMessage("My favourite colour is cerulean. Got it?"))
	...
	for i, task := range work {
		fmt.Printf("  task %d: %s\n", i+1, task)
		result, err := rt.Run(ctx, session, hwcloud.UserMessage(task))
		if err != nil {
	...
	result, err := rt.Run(ctx, session,
		hwcloud.UserMessage("What is my favourite colour? I told you at the very beginning."))
	if err != nil {
		fmt.Fprintf(os.Stderr, "ERROR: %v\n", err)
	} else if result != nil {
		fmt.Printf("  Agent: %s\n", result.FinalOutput)
