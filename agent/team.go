...
type AgentRunner interface {
	RunWithPrefix(ctx context.Context, session hwcloud.Session, prefix []hwcloud.Message, input hwcloud.Message) (*hwcloud.RunResult, error)
	RunStreamWithPrefix(ctx context.Context, session hwcloud.Session, prefix []hwcloud.Message, input hwcloud.Message) <-chan hwcloud.StreamEvent
}
...
//
//	team := hwcloud.NewTeam(
//	    hwcloud.WithTeamAgent("planner", "breaks down tasks", plannerAgent),
//	    hwcloud.WithTeamAgent("coder", "writes code", coderAgent),
//	    hwcloud.WithTeamRouter(hwcloud.NewLLMRouter(model)),
//	    hwcloud.WithTeamMaxHandoffs(12),
//	)
...
// WithTeamObserver sets a hwcloud.RunObserver that receives stage events during team
// execution (agent starts, handoffs, etc.).
func WithTeamObserver(o hwcloud.RunObserver) TeamOption {
...
func (t *Team) Run(ctx context.Context, session hwcloud.Session, input hwcloud.Message) (*TeamResult, error) {
	if len(t.agents) == 0 {
...
func (t *Team) RunStream(ctx context.Context, session hwcloud.Session, input hwcloud.Message) <-chan TeamEvent {
...
type teamRunner struct {
	...
	chain       []HandoffEntry
	runMessages []hwcloud.Message // all messages from this team run (used as prefix for next agent)
	totalTurns  int
	totalUsage  hwcloud.Usage
...
func (tr *teamRunner) observeStage(ctx context.Context, ev hwcloud.StageEvent) {
...
func (tr *teamRunner) run(ctx context.Context, session hwcloud.Session, input hwcloud.Message, ch chan<- TeamEvent) (*TeamResult, error) {
	...
	ctx = hwcloud.WithRunInfo(ctx, hwcloud.RunInfo{RunID: tr.runID, TurnID: -1, ParentRunID: prev.RunID})
	...
	if rr.Fallback {
		tr.observeStage(ctx, hwcloud.StageEvent{
			Name: hwcloud.StageTeamRoute, Phase: "leave",
	...
	for {
		...
		if runErr != nil {
			...
			tr.observeStage(ctx, hwcloud.StageEvent{
				Name: hwcloud.StageTeamAgent, Phase: "leave",
		...
		if len(tr.runMessages) > 128 {
			...
			for len(tr.runMessages) > 0 && tr.runMessages[0].Role == hwcloud.RoleTool {
			...
			for len(tr.runMessages) > 0 && tr.runMessages[0].Role == hwcloud.RoleAssistant && len(tr.runMessages[0].ToolCalls) > 0 {
				tr.runMessages = tr.runMessages[1:]
...
func (tr *teamRunner) runAgentStreaming(ctx context.Context, runner AgentRunner, session hwcloud.Session, input hwcloud.Message, ch chan<- TeamEvent) (*hwcloud.RunResult, error) {
...
func (tr *teamRunner) buildTeamContext(myName string) TeamContext {
	...
	var handoffTools []hwcloud.Tool
	if !tr.forceFinal {
		handoffTools = make([]hwcloud.Tool, 0, len(members))
...
// handoffTool is a hwcloud.Tool implementation for transfer_to_<name>.
...
func (t *handoffTool) Definition() hwcloud.FunctionDefinition {
	return hwcloud.FunctionDefinition{
		...
		EndTurn:     true,
		Parameters:  hwcloud.SchemaOf[HandoffParams](),
	}
}

func (t *handoffTool) Execute(_ context.Context, args json.RawMessage) *hwcloud.ToolResult {
	var params struct {
		Message string `json:"message"`
	}
	if err := json.Unmarshal(args, &params); err != nil {
		return hwcloud.ErrorResult(fmt.Errorf("transfer_to_%s: %w", t.target, err), false, "")
	}

	t.record(t.target, params.Message)
	return &hwcloud.ToolResult{Content: fmt.Sprintf("Transferred to %s.", t.target)}
}
