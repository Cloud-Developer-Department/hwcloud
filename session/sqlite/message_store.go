...
// budget.
func (m *MessageStore) WithSummarizer(s hwcloud.Summarizer) *MessageStore {
	m.summarizer = s
	return m
}
...
func (m *MessageStore) Append(ctx context.Context, sessionID string, msg hwcloud.Message) error {
...
func (m *MessageStore) Recent(ctx context.Context, sessionID string, n int, offset int) ([]hwcloud.Message, error) {
	...
	for len(msgs) > 0 && msgs[0].Role == hwcloud.RoleTool {
		msgs = msgs[1:]
	}
...
func (m *MessageStore) RecentAfter(ctx context.Context, sessionID string, throughIndex, n int) ([]hwcloud.Message, error) {
	if throughIndex < 0 {
		throughIndex = 0
	}
	if n <= 0 {
...
func (m *MessageStore) Compact(ctx context.Context, sessionID string, throughIndex int, messages []hwcloud.Message) error {
	if m.summarizer == nil {
		return nil
	}
	...
	safeIdx := hwcloud.SafeCompressionBoundary(all, throughIndex)
...
func (m *MessageStore) Compressed(ctx context.Context, sessionID string) (*hwcloud.CompressedContext, error) {
	...
	var cc hwcloud.CompressedContext
...
func (m *MessageStore) storeCompressed(ctx context.Context, sessionID string, cc *hwcloud.CompressedContext) error {
	b, _ := json.Marshal(cc)
...
func scanMessages(rows *sql.Rows) ([]hwcloud.Message, error) {
	var msgs []hwcloud.Message
	for rows.Next() {
		var id int64
...
func rowToMessage(role, name, content, contentParts, toolCalls, toolCallID, reasoningContent string) hwcloud.Message {
	msg := hwcloud.Message{
		Role:             hwcloud.Role(role),
		Name:             name,
		Content:          content,
	...
	if contentParts != "" {
		var parts []hwcloud.ContentPart
		if json.Unmarshal([]byte(contentParts), &parts) == nil {
			msg.ContentParts = parts
		}
	}
	if toolCalls != "" && toolCalls != "[]" {
		var calls []hwcloud.ToolCall
