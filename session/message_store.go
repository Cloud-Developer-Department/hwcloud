...
// hwcloud.Memory interface — see the Context Architecture design.
...
type SessionStore interface {
	...
	Append(ctx context.Context, sessionID string, msg hwcloud.Message) error

	// Recent returns up to n most recent messages, skipping the first
	// offset messages from the end. offset=0 returns the latest n.
	Recent(ctx context.Context, sessionID string, n int, offset int) ([]hwcloud.Message, error)
	...
	// a backend row id.
	RecentAfter(ctx context.Context, sessionID string, throughIndex, n int) ([]hwcloud.Message, error)
...
type Compressor interface {
	...
	Compact(ctx context.Context, sessionID string, throughIndex int, messages []hwcloud.Message) error

	// Compressed returns the stored CompressedContext, or nil if none exists.
	Compressed(ctx context.Context, sessionID string) (*hwcloud.CompressedContext, error)
}
